			...
			if global_state.friends[key]["Name"] == name:
				if global_state.snitchtank != {} and own_snitch==False:
					...
					if v_snitch_en != None:
						...
						snitch = PolarCoordinates(v_us,v_snitch_en)
...
# 					diff = old_angle - info['angle']
# 					moved = PolarCoordinates(old_v,v_us)
# 					diff = np.rad2deg(np.sin(moved['distance']/info['distance']))
# 					print('-------DIFF----{}'.format(diff))
					stream.sendMessage(ServerMessageTypes.TURNTURRETTOHEADING, {'Amount':int(info['angle'])})
# 					if abs(int(info['angle']-diff) - global_state.friends[key]['TurretHeading'])<2:
					stream.sendMessage(ServerMessageTypes.FIRE)
# 					v_old = v_us
